...
public class QacctTest extends TestCase {
    ...
    public void testChecks() {
        ...
        result = Qacct.invoke(new File("src/test/bin/qacct-ok").getAbsolutePath(), "12345", "foogly");
        assertFalse(Qacct.isFailed(result));
        assertFalse(Qacct.isExitError(result));

        result = Qacct.invoke(new File("src/test/bin/qacct-failed").getAbsolutePath(), "12345", "foogly");
        assertTrue(Qacct.isFailed(result));
        assertFalse(Qacct.isExitError(result));

        result = Qacct.invoke(new File("src/test/bin/qacct-exit-error").getAbsolutePath(), "12345", "foogly");
        assertFalse(Qacct.isFailed(result));
